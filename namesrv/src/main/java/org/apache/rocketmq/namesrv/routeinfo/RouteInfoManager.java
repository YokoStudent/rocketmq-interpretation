...
/**
 * Namesrv用来存储路由的基础信息都放在RouteInfoManager类中，RouteInfoManager也可以看成是Namesrv的资源类
 */
public class RouteInfoManager {
     ...
     * brokerLiveTable: {
     *     "192.168.56.1:10000": {
     *         "lastUpdatetimestamp": 1218270318980,
     *         "dataVersion": versoinObj,
     *         "channel": channelObj,
    ...
    /**
     * Broker上的FilterService列表　用于类模式消息过滤
     */
    ...
    /**
     * 处理broker的心跳包
     * 判断broker集群是否存在，不存在则创建并添加到clusterAddrTable集合中
     */
    public RegisterBrokerResult registerBroker(
        ...
        try {
            try {
                ...
                // 1.集群是否存在，不存则添加到clusterAddrTable集合中
                ...
                // 2.broker基本信息是否存在，不存在则新建一个brokerData新加到brokerAddrTable，并标识首次注册
                ...
                // 3.如果broker是master，而且brokerTopic发生变化或者是初次注册，则更新topicQueueTable消息队列路由信息
    ...
    /**
     * 根据topic获取TopicRouteData，包含brokerData、filterServer、queueData
     */
    ...
    /**
     * broker失效，移除所有相关信息
     * brokerLiveTable、filterServerTable、brokerAddrTable、clusterAddrTable、topicQueueTable
     */
